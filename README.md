...
* `bootlm` is a function for calculating confidence intervals and *p*-values for the regression coefficients or estimated marginal means from a linear model. The usage of this function is similar to `anovan`. This function uses `bootwild` or `bootbayes` and is robust to heteroscedasticity and supports cluster/block resampling.
...
* `randtest2` is a function for calculating permutation or randomization tests for comparing independent or paired distributions using the Wassertstein metric. A feature of this function is that one can define the sampling units for clustered resampling in the case of nested experimental designs. 

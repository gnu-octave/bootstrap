...
* `bootbayes` performs Bayesian bootstrap and calculates bootstrap bias, standard error and credible intervals for coefficients from a linear regression model. The interval type supported is simple percentile.
...
* `bootcoeff` (Octave only) is a function for calculating Bayesian bootstrap credible intervals for the regression coefficients of a linear model fit using `anovan` or `fitlm`. This function uses `bootbayes`.
* `bootemm` (Octave only) is a function for calculating Bayesian bootstrap credible intervals for the estimated marginal means of a linear model fit using `anovan` or `fitlm`. This function uses `bootbayes`.
